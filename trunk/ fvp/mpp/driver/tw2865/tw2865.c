...
#define INVALID_CHNNO(ChnNo) ((ChnNo) >= chips)
...
{   
    ...
    if (TW2865_PAL == mode)
	{
        for (i=0; i<4; i++)
        {
            ...
            tw2865_byte_write(addr, 0x08 + 0x10*i, 0x12);/* Vertical Delay Register, Low  */
...
int tw2865_ioctl(struct inode *inode, struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	{		
        ...
        {            
			...
			if (analog_cfg.item_sel & TW2865_SET_CONTRAST)//brightness
			{
...
static void tw2865_video_init(unsigned char addr, unsigned char mode)
{
    ...
    for (i=0; i<4; i++)
    {
        ...
        tw2865_byte_write(addr, 0x03 + 0x10*i, 0x19);/* sharpness */
    ...
    tw2865_byte_write(addr, 0xaf, 0x22);
    tw2865_byte_write(addr, 0xb0, 0x22);
