...
/*16cif板的4通道排序*/
unsigned char channel_array_16chn[8] = {0x03,0x21,0xa9,0xcd,0x21,0x87,0xb9,0xdb};
...
static void tw2864_audio_cascade()
{
    ...
    tw2864_byte_write(TW2864B_I2C_ADDR, 0x82, 0x20);
    tw2864_byte_write(TW2864C_I2C_ADDR, 0x82, 0x00);
    tw2864_byte_write(TW2864D_I2C_ADDR, 0x82, 0x20);
...
static void channel_alloc(unsigned char chip_addr,unsigned char ch)
{
    ...
    else if(ch == 4)
    {
		...
		{
		    tw2864_byte_write(chip_addr,0xd3+i,0);
		    tw2864_byte_write(chip_addr,0xd3+i,channel_array_16chn[i]);
		}
    }
}
